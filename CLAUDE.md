...
- **`.agents/skills`** — Repository-local Codex skill fallback for headless workers launched inside this repo/worktree
...
- **Scoped session permissions.** Claude sessions run in `dontAsk` mode with project-specific allow/deny rules. Generated by `/autoboard:task-manifest` at `docs/autoboard/{slug}/session-permissions.json`. Fallback: `config/default-session-permissions.json`. Codex currently uses the nearest documented non-interactive sandbox mode instead of translating this manifest one-for-one. Opt out: `skip-permissions: true` in manifest.
