...
**For each selected dimension**, resolve the absolute path to its `standards/dimensions/{name}.md` file. Use Glob to find it: `glob("**/standards/dimensions/{name}.md")`. Pass the resolved path to the dimension agent. Do NOT paste the file content into the prompt -- the agent has Read tool access.
