...
8. **Auth pre-check** (only if ALL of these are true: `auth-strategy` is not `none`, test credentials are configured, AND at least one acceptance criterion requires authenticated access -- e.g., references logging in, dashboard, user-specific data, or protected routes): Navigate to the login page and attempt sign-in with the test credentials. If login succeeds, continue. If login fails, report as criterion FAIL: "Test user authentication failed -- auth-strategy is '{strategy}' but test credentials could not log in. Verify qa-setup created the test user correctly." This is NOT infrastructure failure -- failed auth login routes through the normal failure/fixer path, not the infrastructure allowlist. Skip this step entirely if no acceptance criteria require authentication -- the login flow may not exist yet in early layers.
