...
The same PASS/FAIL rules apply. If the final QA fails, invoke `/autoboard:qa-fixer` — the fixer owns the retry loop (up to 5 rounds, 3 consecutive non-progress cap) and dispatches sequential fixers before asking the user.
