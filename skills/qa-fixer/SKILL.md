...
2. Invoke /autoboard:debug via the Skill tool to trace root cause.
...
- /autoboard:debug — mandatory before attempting fixes (root cause investigation)
...
Each fixer must invoke `/autoboard:debug` and trace root cause before implementing — no blind retries. Each attempt gets a new worktree (`-qa-fix-L{N}-attempt{M}`).
